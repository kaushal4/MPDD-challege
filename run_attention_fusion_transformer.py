...
def calculate_class_weights(labels_np: np.ndarray, num_classes: int) -> Optional[List[float]]:
    ...
    try:
        ...
        # Check if the number of unique labels found matches num_classes expected
        # This helps catch issues early if a class is entirely missing from the training set
        ...
        weights = []
        # Ensure weights are calculated for indices 0 to num_classes-1
        for i in range(num_classes):
            count = class_counts.get(i, 0) # Default to 0 if class index 'i' is not in counts
            if count == 0:
                # Handle missing class - Assign a default weight or raise error?
                # Assigning 1.0 might be safer than a very large number initially.
        ...
        # Optional normalization (e.g., scale weights relative to the smallest weight)
        # min_weight = min(w for w in weights if w > 0) # Find min non-zero weight
        # if min_weight > 0:
...
# train_epoch (Unchanged from your version)
...
# evaluate (Unchanged from your version)
...
valid_transformer_configs = [(64, 2), (64, 4), (128, 2), (128, 4), (128, 8)]
...
def objective(trial, full_train_dataset, config, computed_weights): # Added computed_weights arg
    ...
    weight_decay = trial.suggest_float("weight_decay", 1e-5, 1e-1, log=True) # Wider range?
    ...
    mlp_dropout = trial.suggest_float("mlp_dropout", 0.2, 0.7, step=0.1)
    batch_size = config.batch_size
    gamma = trial.suggest_float("gamma", 0.5, 5.0, step=0.5) # Tune gamma

    # --- Cross-Validation Setup ---
    # (Using robust label extraction)
    ...
    for fold, (train_split_idx, val_split_idx) in enumerate(skf.split(valid_indices, labels_np)):
        ...
        try:
            model = TransformerLateFusion( # Ensure correct model name
                audio_dim=audio_dim, video_dim=video_dim, pers_dim=pers_dim, num_classes=num_classes,
                transformer_embed_dim=transformer_embed_dim, transformer_nhead=transformer_nhead,
        ...
        best_fold_f1_macro = 0.0; epochs_no_improve = 0; patience = 5;
...
if __name__ == '__main__':
    ...
    try:
        ...
        final_model = TransformerLateFusion( # Ensure correct model name
            audio_dim=config.audio_dim, video_dim=config.video_dim, pers_dim=config.pers_dim, num_classes=config.num_classes,
            transformer_embed_dim=final_transformer_embed_dim, transformer_nhead=final_transformer_nhead, # Use unpacked values
            transformer_num_layers=best_params['transformer_num_layers'], transformer_dropout=best_params['transformer_dropout'],
            mlp_hidden_dim=best_params['mlp_hidden_dim'], mlp_dropout=best_params['mlp_dropout'],
            max_seq_len=config.feature_max_len
        ).to(config.device)
    except KeyError as e: print(f"Error: Missing hyperparameter '{e}'."); exit()
    ...
    if torchinfo:
        ...
        try:
             summary_model = TransformerLateFusion( # Instantiate with final params
                 audio_dim=config.audio_dim, video_dim=config.video_dim, pers_dim=config.pers_dim, num_classes=config.num_classes,
                 transformer_embed_dim=final_transformer_embed_dim, transformer_nhead=final_transformer_nhead,
                 transformer_num_layers=best_params['transformer_num_layers'], transformer_dropout=best_params['transformer_dropout'],
    ...
    best_final_val_f1_macro = 0.0; best_epoch = -1; epochs_no_improve_final = 0; patience_final = 10;
    ...
    if best_epoch != -1 and os.path.exists(config.model_save_path):
        try:
            # Get embed_dim/nhead from best params again for eval model
            ...
            eval_model = TransformerLateFusion( # Ensure correct model name
                audio_dim=config.audio_dim, video_dim=config.video_dim, pers_dim=config.pers_dim, num_classes=config.num_classes,
                transformer_embed_dim=eval_transformer_embed_dim, transformer_nhead=eval_transformer_nhead,
                transformer_num_layers=best_params['transformer_num_layers'], transformer_dropout=best_params['transformer_dropout'],
                mlp_hidden_dim=best_params['mlp_hidden_dim'], mlp_dropout=best_params['mlp_dropout'],
                max_seq_len=config.feature_max_len
            ).to(config.device)
        ...
        except KeyError as e: print(f"Error loading best model: Missing key '{e}'.");
